...
class ResNet50ForFasterrcnn(nn.Cell):
    r"""ResNet model class, based on
    `"Deep Residual Learning for Image Recognition" <https://arxiv.org/abs/1512.03385>`_
    ...
    def forward_features(self, x: Tensor) -> Tensor:
        ...
        x = self.layer1(x)  # TODO: attention!!!
        x = self.layer2(x)
        x = self.layer3(x)
        # x = self.layer4(x)
        return x
...
class Res5Head(nn.Cell):
    def __init__(self, depth):
        ...
        self.input_channels, channels = [1024, 512]
        if depth < 50:
...
def build_resnet50_for_fasterrcnn():
    model = ResNet50ForFasterrcnn(Bottleneck, [3, 4, 6], in_channels=3)
    return model


if __name__ == '__main__':
    # res5head = Res5Head(depth=50)
    # print(res5head)
    # print(model)
